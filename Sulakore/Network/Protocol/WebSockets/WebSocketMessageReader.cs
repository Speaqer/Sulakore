...
namespace Sulakore.Network.Protocol.WebSockets
{
    ...
    public class WebSocketMessageReader
    {
        ...
        public ValueTask<WebSocketMessageReadResult> ReadAsync(CancellationToken cancellationToken = default)
        {
            if (_awaitingHeader)
            {
        ...
        private async ValueTask<WebSocketMessageReadResult> DoReadHeaderRequiredAsync(ValueTask<WebSocketReadFrame> readTask, CancellationToken cancellationToken)
        {
        ...
        /// <param name="cancellationToken">A cancellation token.</param>
        /// <returns>A MessageReadResult.</returns>
        private ValueTask<WebSocketMessageReadResult> ReadPayloadAsync(CancellationToken cancellationToken)
        {
            ...
            {
                return new ValueTask<WebSocketMessageReadResult>(
                    new WebSocketMessageReadResult(new ReadOnlySequence<byte>(_buffer.WrittenMemory), endOfMessage, _isCanceled, _isCompleted));
            }
            else
            {
                return new ValueTask<WebSocketMessageReadResult>(new WebSocketMessageReadResult(_currentSequence, endOfMessage, _isCanceled, _isCompleted));
            }
        }
        ...
        private async ValueTask<WebSocketMessageReadResult> CreateMessageReadResultAsync(ValueTask<ProtocolReadResult<ReadOnlySequence<byte>>> readTask, CancellationToken cancellationToken)
        {
            ...
            if (_buffer.UnconsumedWrittenCount > 0)
            {
                return new WebSocketMessageReadResult(new ReadOnlySequence<byte>(_buffer.WrittenMemory), endOfMessage, _isCanceled, _isCompleted);
            }
            else
            {
                return new WebSocketMessageReadResult(_currentSequence, endOfMessage, _isCanceled, _isCompleted);
            }
        }
