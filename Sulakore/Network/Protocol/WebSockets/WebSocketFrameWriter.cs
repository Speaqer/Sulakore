...
namespace Sulakore.Network.Protocol.WebSockets
{
    ...
    /// </summary>
    internal struct WebSocketFrameWriter : IMessageWriter<WebSocketWriteFrame>
    {
        /// <summary>
        ...
        public void WriteMessage(WebSocketWriteFrame message, IBufferWriter<byte> output)
        {
            if (message.Header.PayloadLength != (ulong)message.Payload.Length)
            {
                throw new Exception($"Header payload length ({message.Header.PayloadLength}) does not equal supplied payload length ({message.Payload.Length})");
            }
